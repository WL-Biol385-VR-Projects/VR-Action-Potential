    ...
    public class VRTK_SnapDropZone : MonoBehaviour
    {
        ...
        private void SnapObject(Collider collider)
        {
            ...
            if (willSnap && !isSnapped && ioCheck)
            {
                ...
                //Set the position and rotation of the interactable object to the snap drop zone highlight object
                ioCheck.transform.position = highlightObject.transform.position;
                ioCheck.transform.rotation = highlightObject.transform.rotation;

                // If apply scaling is checked then use the drop zone scale to resize the object
                var newLocalScale = (applyScalingOnSnap ? transform.localScale : Vector3.zero);
                ioCheck.ToggleSnapDropZone(this, true, newLocalScale);
            }
